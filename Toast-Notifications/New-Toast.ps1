...
.PARAMETER Config
    Specify the path for the custommessage.xml. If none is specified, the script uses the local config-corona.xml
...
# Default is executing directory. In this case, the config-corona.xml must exist in same directory as the New-CoronaToast.ps1 file
if (-NOT($Config)) {
    Write-Log -Message "No config file set as parameter. Using local config file"
    $Config = Join-Path ($global:ScriptPath) "config-corona.xml"
}

# Load config-corona.xml
if (Test-Path $Config) {
    try { 
...
# Check if toast is enabled in config-corona.xml
