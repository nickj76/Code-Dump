...
(
    ...
    [Switch]$Snooze,
    [string]$XMLSource = "https://github.com/nickj76/Scripts/blob/master/Toast-Notifications/Low-Diskspace.xml",
    [String]$ToastGUID
)
...
If ($XMLValid -eq $True) {
    ...
    #Images
    $BadgeImage = "file:///c:\toast\badgeimage.jpg"
    $HeroImage = "file:///c:\toast\heroimage.jpg"
    ...
    If (([System.Security.Principal.WindowsIdentity]::GetCurrent()).Name -eq "NT AUTHORITY\SYSTEM") {
        ...
        $Task_Settings = New-ScheduledTaskSettingsSet -Compatibility V1 -DeleteExpiredTaskAfter (New-TimeSpan -Seconds 6000) -AllowStartIfOnBatteries
    ...
    If (!(([System.Security.Principal.WindowsIdentity]::GetCurrent()).Name -eq "NT AUTHORITY\SYSTEM")) {
        ...
        $Hour = (Get-Date).Hour
        If ($Hour -lt 12) { $CustomHello = "Good Morning " }
        ElseIf ($Hour -gt 16) { $CustomHello = "Good Evening " }
        Else { $CustomHello = "Good Afternoon " }
...
"@
	...
	New-Item -ItemType "file" -Path "c:\logfiles\gp-toast-notification.txt"
}
