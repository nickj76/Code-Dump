    ...
    while ($nextUri) {
        ...
        for ($attempt = 0; $attempt -le $maxRetries; $attempt++) {
            try {
                ...
                # Try to extract the Graph error body for non-retryable failures
                $graphError = ""
                try {
                    if ($_.ErrorDetails -and $_.ErrorDetails.Message) {
                        $graphError = " — " + $_.ErrorDetails.Message
                    } elseif ($respProp) {
                        $content = $respProp.Value.Content
                        if ($content) { $graphError = " — " + $content.ReadAsStringAsync().GetAwaiter().GetResult() }
                    }
                } catch { }
                Write-Warning "Graph request failed for $safeUri (HTTP $statusCode)$graphError"
...
function Get-ItemPlatform {
    ...
    if (
        $CategoryKey -eq 'scripts' -or
        $CategoryKey -eq 'remediations' -or
        $CategoryKey -eq 'autopilotProfiles' -or
        $CategoryKey -eq 'enrollmentRestrictions' -or
        $CategoryKey -eq 'esp' -or
        $CategoryKey -eq 'featureUpdates' -or
        $CategoryKey -eq 'qualityUpdates' -or
        $CategoryKey -eq 'driverUpdates' -or
        $CategoryKey -eq 'endpointSecurity' -or
        $CategoryKey -eq 'diskEncryption' -or
        $CategoryKey -eq 'attackSurfaceReduction' -or
        $CategoryKey -eq 'groupPolicyConfigs'
    ) {
        return "Windows"
    }
...
function Test-CategoryItemMatch {
    param([object]$Item, [string]$CategoryKey)

    if ($CategoryKey -ne 'enrollmentRestrictions' -and $CategoryKey -ne 'esp') {
        return $true
    }
    ...
    $t = "$odataType".ToLower()
    if ($CategoryKey -eq 'enrollmentRestrictions') {
        return ($t -match 'platformrestrictionsconfiguration|enrollmentlimitconfiguration')
    }

    return ($t -match 'enrollmentcompletionpageconfiguration')
}

function Get-AssignmentsForGroup {
    param([string]$GroupId)

    # $select only lists data properties — 'assignments' is a navigation property accessed via $expand only
    $categories = @{
        configurations  = "/beta/deviceManagement/deviceConfigurations?`$select=id,displayName,description&`$expand=assignments"
        compliance      = "/beta/deviceManagement/deviceCompliancePolicies?`$select=id,displayName,description&`$expand=assignments"
        settingsCatalog = "/beta/deviceManagement/configurationPolicies?`$select=id,name,description&`$expand=assignments"
        applications    = "/beta/deviceAppManagement/mobileApps?`$select=id,displayName,description&`$expand=assignments&`$filter=isAssigned eq true"
        scripts         = "/beta/deviceManagement/deviceManagementScripts?`$select=id,displayName,description&`$expand=assignments"
        remediations    = "/beta/deviceManagement/deviceHealthScripts?`$select=id,displayName,description&`$expand=assignments"
        autopilotProfiles      = "/beta/deviceManagement/windowsAutopilotDeploymentProfiles?`$select=id,displayName,description&`$expand=assignments"
        enrollmentRestrictions = "/beta/deviceManagement/deviceEnrollmentConfigurations?`$select=id,displayName,description&`$expand=assignments"
        esp                    = "/beta/deviceManagement/deviceEnrollmentConfigurations?`$select=id,displayName,description&`$expand=assignments"
        featureUpdates         = "/beta/deviceManagement/windowsFeatureUpdateProfiles?`$select=id,displayName,description&`$expand=assignments"
        qualityUpdates         = "/beta/deviceManagement/windowsQualityUpdateProfiles?`$select=id,displayName,description&`$expand=assignments"
        driverUpdates          = "/beta/deviceManagement/windowsDriverUpdateProfiles?`$select=id,displayName,description&`$expand=assignments"
        endpointSecurity       = "/beta/deviceManagement/intents?`$select=id,displayName,description&`$expand=assignments"
        diskEncryption         = "/beta/deviceManagement/intents?`$filter=templateId eq 'a239407c-698d-4ef8-b314-e3ae409204b8'&`$expand=assignments"
        attackSurfaceReduction = "/beta/deviceManagement/intents?`$filter=templateId eq 'e8c053d6-9f01-4b86-b48c-e9033826d83f'&`$expand=assignments"
        groupPolicyConfigs     = "/beta/deviceManagement/groupPolicyConfigurations?`$select=id,displayName,description&`$expand=assignments"
    }
    ...
    foreach ($cat in $categories.GetEnumerator()) {
        ...
        foreach ($item in $items) {
            if (-not (Test-CategoryItemMatch -Item $item -CategoryKey $cat.Key)) { continue }
            ...
            foreach ($assignment in $itemAssignments) {
                ...
                if ($isGroupMatch -or $isAllDevices -or $isAllUsers) {
                    $itemId   = Get-SafeValue $item 'id'
                    $dedupKey = "$itemId|$targetType|$targetGroupId"
                    if (-not $seenKeys.Add($dedupKey)) { continue }
...
function Get-GroupMembers {
    <#
    .SYNOPSIS
        Returns the direct members of a group (users, devices, nested groups).
    #>
    param([Parameter(Mandatory)][string]$GroupId)

    $uri = "/v1.0/groups/$GroupId/members?`$select=id,displayName,userPrincipalName,deviceId&`$top=999"
    $raw = @(Invoke-GraphPaginated -Uri $uri -SilentErrors)
    $members = @($raw | ForEach-Object {
        $odataType = Get-SafeValue $_ '@odata.type'
        $kind = switch -Wildcard ($odataType) {
            '*user*'   { "User"   }
            '*device*' { "Device" }
            '*group*'  { "Group"  }
            default    { "Other"  }
        ...
        @{
            id          = Get-SafeValue $_ 'id'
            displayName = Get-SafeValue $_ 'displayName'
            upn         = Get-SafeValue $_ 'userPrincipalName'
            kind        = $kind
        }
    })
    return $members
...
function Get-ManagedDevicesByName {
    <#
    .SYNOPSIS
        Searches Intune managed devices by name (partial match, max 50 results).
    #>
    param(
    ...
    # Only allow safe characters in the device name to prevent Graph injection
    if ($SearchName -notmatch '^[\w\s\.\-]+$') {
        throw "Device name contains invalid characters."
    }

    $encoded = [Uri]::EscapeDataString($SearchName)
    # Graph managedDevices supports startsWith() and eq on deviceName, but NOT contains().
    $filter = "startsWith(deviceName,'$encoded')"
    if ($CorporateOnly) { $filter = "$filter and managedDeviceOwnerType eq 'company'" }
    $uri = "/beta/deviceManagement/managedDevices?`$filter=$([Uri]::EscapeDataString($filter))&`$select=id,deviceName,azureADDeviceId,operatingSystem,osVersion,userPrincipalName,lastSyncDateTime,complianceState,managedDeviceOwnerType&`$top=50"
    $devices = @(Invoke-GraphPaginated -Uri $uri -SilentErrors)
    return $devices
}
...
function Get-AllManagedDevices {
    <#
    .SYNOPSIS
        Returns all Intune managed devices sorted by deviceName.
        Used to populate the Devices sidebar list on first switch to Devices mode.
    #>
    param([switch]$CorporateOnly)

    $filter = if ($CorporateOnly) { "managedDeviceOwnerType eq 'company'" } else { $null }
    $uri = "/beta/deviceManagement/managedDevices?`$select=id,deviceName,operatingSystem,osVersion,userPrincipalName,lastSyncDateTime,complianceState,managedDeviceOwnerType&`$top=100"
    if ($filter) { $uri += "&`$filter=$([Uri]::EscapeDataString($filter))" }
    $devices = @(Invoke-GraphPaginated -Uri $uri)
    if ($devices.Count -gt 0) {
        $devices = @($devices | Sort-Object { $_.deviceName })
    }
    $devices
}

function Get-AssignmentsForDevice {
    <#
    .SYNOPSIS
        Returns all Intune assignments that apply to a given managed device.
        Resolves the device's Entra group memberships transiently, then scans
        all 6 policy categories for matching assignments (same logic as
        Get-AssignmentsForGroup but matched against the full group set).
    #>
    param([Parameter(Mandatory)][string]$ManagedDeviceId)

    # 1. Fetch the managed device to obtain its Entra (Azure AD) device GUID
    $deviceUri = "/beta/deviceManagement/managedDevices/$ManagedDeviceId`?`$select=id,deviceName,azureADDeviceId,operatingSystem,osVersion,userPrincipalName,lastSyncDateTime,complianceState"
    $device = Invoke-MgGraphRequest -Method GET -Uri $deviceUri
    if (-not $device) { throw "Device not found." }

    $azureADDeviceId = Get-SafeValue $device 'azureADDeviceId'
    if (-not $azureADDeviceId) { throw "Device '$ManagedDeviceId' has no Entra device ID (azureADDeviceId is empty)." }

    # 2. Resolve the Entra object ID from the device GUID
    $entraDeviceUri = "/v1.0/devices?`$filter=deviceId eq '$azureADDeviceId'&`$select=id"
    $entraResult    = Invoke-MgGraphRequest -Method GET -Uri $entraDeviceUri
    $entraDevices   = @(Get-SafeValue $entraResult 'value')
    if ($entraDevices.Count -eq 0) {
        throw "Entra device object not found for azureADDeviceId '$azureADDeviceId'."
    }
    $entraObjectId = Get-SafeValue $entraDevices[0] 'id'

    # 3. Get transitive group memberships for the Entra device object
    $membershipUri = "/v1.0/devices/$entraObjectId/transitiveMemberOf/microsoft.graph.group?`$select=id,displayName&`$top=999"
    $deviceGroups  = @(Invoke-GraphPaginated -Uri $membershipUri -SilentErrors)
    $deviceGroupIds = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
    foreach ($g in $deviceGroups) {
        $gid = Get-SafeValue $g 'id'
        if ($gid) { [void]$deviceGroupIds.Add($gid) }
    }

    # 3b. Also fetch the primary user's group memberships so that user-targeted
    #     policies (e.g. compliance, apps assigned to user groups) are included.
    $userGroups    = @()
    $userGroupIds  = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
    $upn = Get-SafeValue $device 'userPrincipalName'
    if ($upn) {
        try {
            $encodedUpn  = [Uri]::EscapeDataString($upn)
            $userMemUri  = "/v1.0/users/$encodedUpn/transitiveMemberOf/microsoft.graph.group?`$select=id,displayName&`$top=999"
            $userGroups  = @(Invoke-GraphPaginated -Uri $userMemUri -SilentErrors)
            foreach ($g in $userGroups) {
                $gid = Get-SafeValue $g 'id'
                if ($gid) { [void]$userGroupIds.Add($gid) }
            }
        }
        catch {
            Write-Warning "Could not fetch user group memberships for $upn : $($_.Exception.Message)"
        }
    }

    # Combined lookup map: id -> displayName (device groups + user groups)
    $groupDisplayNames = @{}
    foreach ($g in $deviceGroups + $userGroups) {
        $gid = Get-SafeValue $g 'id'
        $gdn = Get-SafeValue $g 'displayName'
        if ($gid -and -not $groupDisplayNames.ContainsKey($gid)) { $groupDisplayNames[$gid] = $gdn }
    }

    # 4. Scan policy categories (compliance handled separately via deviceCompliancePolicyStates)
    # Note: $select only lists data properties — 'assignments' is a navigation property and must NOT
    # appear in $select; it is returned via $expand=assignments only.
    $categories = @{
        configurations  = "/beta/deviceManagement/deviceConfigurations?`$select=id,displayName,description&`$expand=assignments"
        settingsCatalog = "/beta/deviceManagement/configurationPolicies?`$select=id,name,description&`$expand=assignments"
        applications    = "/beta/deviceAppManagement/mobileApps?`$select=id,displayName,description&`$expand=assignments&`$filter=isAssigned eq true"
    ...
    foreach ($cat in $categories.GetEnumerator()) {
        ...
        try {
            ...
            Write-Warning "Device assignments — category $($cat.Key) failed: $($_.Exception.Message)"
            $_errors[$cat.Key] = $_.Exception.Message
            $result[$cat.Key]  = @()
            continue
        }
        ...
        foreach ($item in $items) {
            ...
            foreach ($assignment in $itemAssignments) {
                ...
                $isAllDevices      = ($targetType -eq '#microsoft.graph.allDevicesAssignmentTarget')
                $isAllUsers        = ($targetType -eq '#microsoft.graph.allLicensedUsersAssignmentTarget')
                $isDeviceGroupMatch = ($targetGroupId -and $deviceGroupIds.Contains($targetGroupId))
                $isUserGroupMatch   = ($targetGroupId -and $userGroupIds.Contains($targetGroupId))
                $isGroupMatch       = $isDeviceGroupMatch -or $isUserGroupMatch

                if (-not ($isGroupMatch -or $isAllDevices -or $isAllUsers)) { continue }

                $itemId   = Get-SafeValue $item 'id'
                # For device view, deduplicate by item ID only — device doesn't care how many ways policy applies
                if (-not $seenKeys.Add($itemId)) { continue }

                # For group assignments, resolve display name and source (device vs user group)
                $viaGroup = ""
                if ($isGroupMatch) {
                    $dn = $groupDisplayNames[$targetGroupId]
                    $viaGroup = if ($dn) { $dn } else { $targetGroupId }
                    if ($isUserGroupMatch -and -not $isDeviceGroupMatch) {
                        $viaGroup = "$viaGroup (user)"
                    }
                }
    ...
    # 5. Compliance: use deviceCompliancePolicyStates for reliable detection.
    #    This works for direct assignments, Policy Set assignments, and All Devices/All Users targets.
    $complianceMatched = [System.Collections.ArrayList]::new()
    try {
        $complianceStates = @(Invoke-GraphPaginated -Uri "/beta/deviceManagement/managedDevices/$ManagedDeviceId/deviceCompliancePolicyStates" -SilentErrors)

        # Bulk-fetch all compliance policies with assignments for group resolution
        $allCompPolicies = @{}
        $allCompItems = @(Invoke-GraphPaginated -Uri "/beta/deviceManagement/deviceCompliancePolicies?`$select=id,displayName,description&`$expand=assignments" -SilentErrors)
        foreach ($p in $allCompItems) {
            $policyLookupId = Get-SafeValue $p 'id'
            if ($policyLookupId) { $allCompPolicies[$policyLookupId] = $p }
        }

        foreach ($state in $complianceStates) {
            $stateId = Get-SafeValue $state 'id'
            if (-not $stateId) { continue }

            # ID format is "policyId:version" — extract the policy GUID
            $policyId        = ($stateId -split ':')[0]
            $stateDisplayName = Get-SafeValue $state 'displayName'
            $policyObj       = $allCompPolicies[$policyId]
            $description     = if ($policyObj) { Get-SafeValue $policyObj 'description' } else { "" }

            # Determine which assignment target matched (best-effort)
            $assignmentType = "Assigned"
            $viaGroup       = ""

            if ($policyObj) {
                $policyAssignments = Get-SafeValue $policyObj 'assignments'
                if ($policyAssignments) {
                    foreach ($assignment in $policyAssignments) {
                        ...
                        $assignmentType = switch ($targetType) {
                            "#microsoft.graph.groupAssignmentTarget"            { "Include" }
                            "#microsoft.graph.exclusionGroupAssignmentTarget"   { "Exclude" }
                            "#microsoft.graph.allDevicesAssignmentTarget"       { "All Devices" }
                            "#microsoft.graph.allLicensedUsersAssignmentTarget" { "All Users" }
                        ...
                        break  # Use first matching assignment target
                    }
                }
            }

            # If still "Assigned" (policy set or unknown assignment), flag it
            if ($assignmentType -eq 'Assigned' -and -not $viaGroup) {
                $viaGroup = "via Policy Set or indirect assignment"
            }

            [void]$complianceMatched.Add(@{
                id             = $policyId
                displayName    = if ($stateDisplayName) { $stateDisplayName } else { "N/A" }
        ...
        Write-Warning "Compliance policy states failed: $($_.Exception.Message)"
        $_errors['compliance'] = $_.Exception.Message
    }
    $result['compliance'] = @($complianceMatched.ToArray())

    $result['_errors'] = $_errors
    $_rawSync = Get-SafeValue $device 'lastSyncDateTime'
    $_syncStr = if ($_rawSync -is [datetime] -or $_rawSync -is [datetimeoffset]) {
        $_rawSync.ToUniversalTime().ToString('o')
    } elseif ($_rawSync) { "$_rawSync" } else { $null }
    $result['_deviceInfo'] = @{
        id                = Get-SafeValue $device 'id'
        deviceName        = Get-SafeValue $device 'deviceName'
        operatingSystem   = Get-SafeValue $device 'operatingSystem'
        osVersion         = Get-SafeValue $device 'osVersion'
        userPrincipalName = Get-SafeValue $device 'userPrincipalName'
        lastSyncDateTime  = $_syncStr
        complianceState   = Get-SafeValue $device 'complianceState'
        groupCount        = $deviceGroups.Count
        groups            = @($deviceGroups | ForEach-Object {
            @{ id = (Get-SafeValue $_ 'id'); displayName = (Get-SafeValue $_ 'displayName') }
        })
    }
    return $result
}
...
function Get-NestedGroupAssignments {
    ...
    $categories = @{
        configurations  = "/beta/deviceManagement/deviceConfigurations?`$select=id,displayName,description,assignments&`$expand=assignments"
        compliance      = "/beta/deviceManagement/deviceCompliancePolicies?`$select=id,displayName,description,assignments&`$expand=assignments"
        settingsCatalog = "/beta/deviceManagement/configurationPolicies?`$select=id,name,description,assignments&`$expand=assignments"
        applications    = "/beta/deviceAppManagement/mobileApps?`$select=id,displayName,description,assignments&`$expand=assignments&`$filter=isAssigned eq true"
        scripts         = "/beta/deviceManagement/deviceManagementScripts?`$select=id,displayName,description,assignments&`$expand=assignments"
        remediations    = "/beta/deviceManagement/deviceHealthScripts?`$select=id,displayName,description,assignments&`$expand=assignments"
        autopilotProfiles      = "/beta/deviceManagement/windowsAutopilotDeploymentProfiles?`$select=id,displayName,description,assignments&`$expand=assignments"
        enrollmentRestrictions = "/beta/deviceManagement/deviceEnrollmentConfigurations?`$select=id,displayName,description,assignments&`$expand=assignments"
        esp                    = "/beta/deviceManagement/deviceEnrollmentConfigurations?`$select=id,displayName,description,assignments&`$expand=assignments"
        featureUpdates         = "/beta/deviceManagement/windowsFeatureUpdateProfiles?`$select=id,displayName,description,assignments&`$expand=assignments"
        qualityUpdates         = "/beta/deviceManagement/windowsQualityUpdateProfiles?`$select=id,displayName,description,assignments&`$expand=assignments"
        driverUpdates          = "/beta/deviceManagement/windowsDriverUpdateProfiles?`$select=id,displayName,description,assignments&`$expand=assignments"
        endpointSecurity       = "/beta/deviceManagement/intents?`$select=id,displayName,description,assignments&`$expand=assignments"
        diskEncryption         = "/beta/deviceManagement/intents?`$filter=templateId eq 'a239407c-698d-4ef8-b314-e3ae409204b8'&`$expand=assignments"
        attackSurfaceReduction = "/beta/deviceManagement/intents?`$filter=templateId eq 'e8c053d6-9f01-4b86-b48c-e9033826d83f'&`$expand=assignments"
        groupPolicyConfigs     = "/beta/deviceManagement/groupPolicyConfigurations?`$select=id,displayName,description,assignments&`$expand=assignments"
    }
    $parentLookup = @{}
...
function Get-AssignedGroupIds {
    ...
    $endpoints = @(
        ...
        "/beta/deviceManagement/windowsAutopilotDeploymentProfiles?`$expand=assignments&`$select=id,assignments"
        "/beta/deviceManagement/deviceEnrollmentConfigurations?`$expand=assignments&`$select=id,assignments"
        "/beta/deviceManagement/windowsFeatureUpdateProfiles?`$expand=assignments&`$select=id,assignments"
        "/beta/deviceManagement/windowsQualityUpdateProfiles?`$expand=assignments&`$select=id,assignments"
        "/beta/deviceManagement/windowsDriverUpdateProfiles?`$expand=assignments&`$select=id,assignments"
    )
    ...
    foreach ($uri in $endpoints) {
        ...
        foreach ($item in $items) {
            $catKey = if ($uri -like '*deviceEnrollmentConfigurations*') {
                $ot = "$(Get-SafeValue $item '@odata.type')".ToLower()
                if ($ot -match 'enrollmentcompletionpageconfiguration') { 'esp' } else { 'enrollmentRestrictions' }
            } else {
                ''
            }
            if ($catKey -and -not (Test-CategoryItemMatch -Item $item -CategoryKey $catKey)) { continue }
            $itemAssignments = Get-SafeValue $item 'assignments'
            if (-not $itemAssignments) { continue }
            foreach ($assignment in $itemAssignments) {
                $target  = Get-SafeValue $assignment 'target'
...
<body>
    ...
    <div class="app-layout">
        ...
        <aside class="sidebar" id="sidebar" data-mode="groups">
            <div class="sidebar-header">
                <div class="sidebar-mode-toggle">
                    <button class="btn-mode active" id="btnModeGroups">Groups</button>
                    <button class="btn-mode" id="btnModeDevices">Devices</button>
                </div>
                <div class="sidebar-header-actions" id="groupHeaderActions">
            ...
            <!-- Group search panel (visible in Groups mode) -->
            <div id="groupSearchBox">
            ...
            <!-- Device search panel (visible in Devices mode) -->
            <div id="deviceSearchPanel" style="display:none">
                <div class="search-box">
                    <svg class="search-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>
                    </svg>
                ...
                <div class="device-ownership-filter">
                    <button class="device-ownership-btn active" id="btnOwnershipCorporate" data-ownership="corporate">Corporate</button>
                    <button class="device-ownership-btn" id="btnOwnershipAll" data-ownership="all">All</button>
                </div>
                <p class="device-search-hint" id="deviceSearchHint"></p>
                <div class="device-search-loading" id="deviceSearchLoading" style="display:none">
                    <div class="spinner"></div>
                </div>
                <ul class="group-list" id="deviceList"></ul>
            </div>
        ...
        <main class="content" id="content">
            ...
            <div class="assignments" id="assignments" style="display:none">
                ...
                <!-- Device info banner (visible when a device is selected) -->
                <div class="device-banner" id="deviceBanner" style="display:none">
                    <div class="device-banner-row">
                        <span class="device-banner-os" id="deviceBannerOs"></span>
                        <span class="device-banner-compliance" id="deviceBannerCompliance"></span>
                    </div>
                    <div class="device-banner-row">
                        <span class="device-banner-user" id="deviceBannerUser"></span>
                        <span class="device-banner-sync" id="deviceBannerSync"></span>
                    </div>
                    <div class="device-banner-groups" id="deviceBannerGroups"></div>
                </div>
                ...
                <div class="category-tabs" id="categoryTabs">
                    ...
                    <button class="tab" data-category="autopilotProfiles">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="14" rx="2"/><line x1="8" y1="20" x2="16" y2="20"/><line x1="12" y1="18" x2="12" y2="20"/></svg>
                        Autopilot Profiles
                        <span class="tab-count" id="countAutopilotProfiles">0</span>
                    </button>
                    <button class="tab" data-category="enrollmentRestrictions">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l8 4v6c0 5-3.4 8.7-8 10-4.6-1.3-8-5-8-10V6l8-4z"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
                        Enrollment Restrictions
                        <span class="tab-count" id="countEnrollmentRestrictions">0</span>
                    </button>
                    <button class="tab" data-category="esp">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l8 4v6c0 5-3.4 8.7-8 10-4.6-1.3-8-5-8-10V6l8-4z"/><polyline points="9 12 11 14 15 10"/></svg>
                        ESP
                        <span class="tab-count" id="countEsp">0</span>
                    </button>
                    <button class="tab" data-category="featureUpdates">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12"/><path d="M7 10l5 5 5-5"/><rect x="3" y="19" width="18" height="2" rx="1"/></svg>
                        Feature Updates
                        <span class="tab-count" id="countFeatureUpdates">0</span>
                    </button>
                    <button class="tab" data-category="qualityUpdates">
                        ...
                        Quality Updates
                        <span class="tab-count" id="countQualityUpdates">0</span>
                    </button>
                    <button class="tab" data-category="driverUpdates">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="7" y="7" width="10" height="10" rx="1"/><path d="M9 3v4M15 3v4M9 17v4M15 17v4M3 9h4M3 15h4M17 9h4M17 15h4"/></svg>
                        Driver Updates
                        <span class="tab-count" id="countDriverUpdates">0</span>
                    </button>
                    <button class="tab" data-category="endpointSecurity">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l8 4v6c0 5-3.4 8.7-8 10-4.6-1.3-8-5-8-10V6l8-4z"/><circle cx="12" cy="12" r="2"/></svg>
                        Endpoint Security
                        <span class="tab-count" id="countEndpointSecurity">0</span>
                    </button>
                    <button class="tab" data-category="diskEncryption">
                        ...
                        Disk Encryption
                        <span class="tab-count" id="countDiskEncryption">0</span>
                    </button>
                    <button class="tab" data-category="attackSurfaceReduction">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l8 4v6c0 5-3.4 8.7-8 10-4.6-1.3-8-5-8-10V6l8-4z"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
                        Attack Surface Reduction
                        <span class="tab-count" id="countAttackSurfaceReduction">0</span>
                    </button>
                    <button class="tab" data-category="groupPolicyConfigs">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                        Group Policy Configs
                        <span class="tab-count" id="countGroupPolicyConfigs">0</span>
                    </button>
                    <button class="tab" data-category="members" style="display:none">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                        Members
                        <span class="tab-count" id="countMembers">0</span>
                    </button>
...
/* ═══════════════════════════════════════════════════════════════════════════
   Assignment Checker — Intune Dashboard
   Palette: Microsoft Intune Navy → Blue → Teal
   ═══════════════════════════════════════════════════════════════════════════ */

:root {
    /* Primary palette — Microsoft Intune blue */
    --primary:        #0067B8;
    --primary-light:  #1578C9;
    --primary-lighter: #5BA8D8;
    --primary-pale:   #EBF3FA;
    --primary-dark:   #005A9E;
    --primary-darker: #004E8C;
    ...
    /* Secondary — sky blue */
    --secondary:      #50B0E6;
    --secondary-dark: #219FD5;

    /* Category accent colours */
    --cat-configurations: #0067B8;
    --cat-compliance:     #107C10;
    --cat-settings:       #008575;
    --cat-applications:   #CA5010;
    --cat-scripts:        #6B69D6;
    --cat-remediations:   #C50F1F;
    --cat-autopilotProfiles:      #005B70;
    --cat-enrollmentRestrictions: #5C2D91;
    --cat-esp:                    #8E562E;
    --cat-featureUpdates:         #0B6A0B;
    --cat-qualityUpdates:         #1A4A86;
    --cat-driverUpdates:          #7A3E2B;
    --cat-endpointSecurity:       #1E3A8A;
    --cat-diskEncryption:         #7C2D12;
    --cat-attackSurfaceReduction: #5E1B34;
    --cat-groupPolicyConfigs:     #003D5C;
    ...
    /* Shadows */
    --shadow-sm:      0 1px 4px rgba(0, 103, 184, 0.08), 0 0 1px rgba(0, 103, 184, 0.06);
    --shadow-md:      0 4px 16px rgba(0, 103, 184, 0.12), 0 1px 4px rgba(0, 103, 184, 0.06);
    --shadow-lg:      0 8px 32px rgba(0, 103, 184, 0.16), 0 2px 8px rgba(0, 103, 184, 0.08);

    /* Header gradient — Intune navy to blue to teal */
    --header-gradient: linear-gradient(135deg, #0A1E3C 0%, #0067B8 55%, #00B7C3 100%);

    /* Focus ring */
    --focus-ring:     rgba(0, 103, 184, 0.15);
}
...
[data-theme="dark"] {
    ...
    --focus-ring:     rgba(91, 168, 216, 0.25);
}
...
.app-header {
    ...
    color: #fff;
    box-shadow: 0 2px 20px rgba(0, 30, 60, 0.40);
}
...
/* CSS-level safety net: hide mode-specific panels based on data-mode attribute.
   Supplements the inline-style toggling in setMode() so that a race between
   async group loading and mode switching can never leave both panels visible. */
.sidebar[data-mode="groups"] #deviceSearchPanel { display: none !important; }
.sidebar[data-mode="devices"] #groupSearchBox,
.sidebar[data-mode="devices"] #groupHeaderActions,
.sidebar[data-mode="devices"] #countFilter,
.sidebar[data-mode="devices"] #groupListSticky,
.sidebar[data-mode="devices"] #groupList,
.sidebar[data-mode="devices"] #sidebarLoading,
.sidebar[data-mode="devices"] #sidebarError { display: none !important; }
...
.btn-mode.active {
    ...
    color: var(--primary);
    font-weight: 600;
    box-shadow: 0 1px 4px rgba(0, 103, 184, 0.15);
}
...
/* ── Group member chips (Members tab) ───────────────────────────────────── */
.member-chip {
    display: inline-flex;
    align-items: center;
    gap: 5px;
...
.member-chip-kind-user   { background: var(--include-bg);  color: var(--include-text); }
.member-chip-kind-device { background: var(--primary-pale); color: var(--primary); }
.member-chip-kind-group  { background: #FEF9C3; color: #854D0E; }
[data-theme="dark"] .member-chip-kind-group { background: #422006; color: #FCD34D; }

.member-chip-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.tab.active {
    ...
    background: var(--primary);
    color: #fff;
    box-shadow: 0 2px 8px rgba(0, 103, 184, 0.30);
}
...
/* Category-specific card left border colours */
.card--configurations::before { background: var(--cat-configurations); }
.card--compliance::before      { background: var(--cat-compliance); }
.card--settingsCatalog::before { background: var(--cat-settings); }
.card--applications::before    { background: var(--cat-applications); }
.card--scripts::before         { background: var(--cat-scripts); }
.card--remediations::before    { background: var(--cat-remediations); }
.card--autopilotProfiles::before      { background: var(--cat-autopilotProfiles); }
.card--enrollmentRestrictions::before { background: var(--cat-enrollmentRestrictions); }
.card--esp::before                    { background: var(--cat-esp); }
.card--featureUpdates::before         { background: var(--cat-featureUpdates); }
.card--qualityUpdates::before         { background: var(--cat-qualityUpdates); }
.card--driverUpdates::before          { background: var(--cat-driverUpdates); }
.card--endpointSecurity::before       { background: var(--cat-endpointSecurity); }
.card--diskEncryption::before         { background: var(--cat-diskEncryption); }
.card--attackSurfaceReduction::before { background: var(--cat-attackSurfaceReduction); }
.card--groupPolicyConfigs::before     { background: var(--cat-groupPolicyConfigs); }
...
.badge-exclude {
    background: var(--exclude-bg);
    color: var(--exclude-text);
    border: 1px solid rgba(197, 15, 31, 0.18);
}
...
[data-theme="dark"] .badge-via-group {
    background: #022C22;
    color: #6EE7B7;
    border-color: rgba(110, 231, 183, 0.2);
}

/* Device search meta line */
.group-item-meta {
    display: block;
    font-size: 11px;
    color: var(--text-secondary);
...
/* Device ownership filter toggle */
.device-ownership-filter {
    display: flex;
    gap: 6px;
    padding: 6px 16px 2px;
}
.device-ownership-btn {
    flex: 1;
    padding: 4px 0;
    font-size: 12px;
    font-weight: 500;
...
.device-ownership-btn.active {
    background: var(--accent);
    color: #fff;
    border-color: var(--accent);
}

/* Device search loading spinner row */
...
.device-search-loading::after {
    content: 'Fetching devices from Intune...';
    font-size: 11.5px;
    color: var(--text-muted);
}
...
.device-banner-row + .device-banner-row {
    margin-top: 2px;
}
.device-banner-os {
    font-weight: 500;
...
.device-banner-compliance.compliance-compliant {
    color: #107C10;
    font-weight: 600;
}
.device-banner-compliance.compliance-noncompliant {
    color: #C50F1F;
    font-weight: 600;
}
.device-banner-groups {
    margin-top: 6px;
    font-size: 11px;
    color: var(--text-secondary);
    border-top: 1px solid var(--border);
...
.orphaned-card::before {
    background: linear-gradient(180deg, var(--cat-applications) 0%, var(--cat-remediations) 100%);
}
...
(function () {
    ...
    // Device search DOM references
    var btnModeGroups        = document.getElementById("btnModeGroups");
    var btnModeDevices       = document.getElementById("btnModeDevices");
    var groupSearchBox       = document.getElementById("groupSearchBox");
    var groupHeaderActions   = document.getElementById("groupHeaderActions");
    var deviceSearchPanel    = document.getElementById("deviceSearchPanel");
    var deviceSearch         = document.getElementById("deviceSearch");
    var deviceList           = document.getElementById("deviceList");
    var deviceSearchLoading  = document.getElementById("deviceSearchLoading");
    var deviceSearchHint     = document.getElementById("deviceSearchHint");
    var deviceBanner         = document.getElementById("deviceBanner");
    var deviceBannerOs       = document.getElementById("deviceBannerOs");
    var deviceBannerCompliance = document.getElementById("deviceBannerCompliance");
    var deviceBannerUser     = document.getElementById("deviceBannerUser");
    var deviceBannerSync     = document.getElementById("deviceBannerSync");
    var deviceBannerGroups   = document.getElementById("deviceBannerGroups");
    ...
    var groupMembersData   = null;  // direct members of the currently selected group
    ...
    var activeMode           = "groups"; // "groups" or "devices"
    var allDevices           = [];
    var devicesLoaded        = false;
    var deviceOwnershipFilter = "corporate"; // "corporate" or "all" (default: corporate)
        ...
        if (_backendKey) {
            try {
                ...
                if (resp.ok) {
                    appMode = "backend";
                    var statusData = {};
                    try { statusData = await resp.json(); } catch (e) {}
                    var acct = (statusData && statusData.account) ? statusData.account : "Connected";
                    showApp();
                    setConnection("connected", acct);
                    loadGroups();
                    return;
                }
            } catch (e) {
    ...
    function populateGroupHeader(group) {
        // Hide device banner when showing a group
    ...
    function populateDeviceHeader(device, deviceInfo) {
        var name = (deviceInfo && deviceInfo.deviceName) || device.deviceName || "Unknown Device";
        selectedGroupName.textContent = name;
        // Remove stale copy buttons
        var existingCopy = selectedGroupName.parentNode.querySelector(".btn-copy-name");
        if (existingCopy) existingCopy.remove();
        var nameCopy = createCopyButton(function () { return name; });
        nameCopy.classList.add("btn-copy-name");
        ...
        var os  = (deviceInfo && deviceInfo.operatingSystem) || device.operatingSystem || "";
        var ver = (deviceInfo && deviceInfo.osVersion) || "";
        selectedGroupDesc.textContent = os + (ver ? " " + ver : "");
        var existingDescCopy = selectedGroupDesc.parentNode.querySelector(".btn-copy-desc");
        if (existingDescCopy) existingDescCopy.remove();

        // Hide dynamic rule row
        if (membershipRuleEl) membershipRuleEl.style.display = "none";

        // Populate device banner
        if (deviceBanner) {
            var compliance = (deviceInfo && deviceInfo.complianceState) || "";
            var user       = (deviceInfo && deviceInfo.userPrincipalName) || device.userPrincipalName || "";
            var rawSync    = deviceInfo && deviceInfo.lastSyncDateTime;
            var lastSync   = "";
            if (rawSync) {
                // Handle both ISO 8601 strings and legacy /Date(ms)/ format
                var _d;
                var _msMatch = /\/Date\((-?\d+)\)\//.exec(rawSync);
                if (_msMatch) { _d = new Date(parseInt(_msMatch[1], 10)); }
                else { _d = new Date(rawSync); }
                if (!isNaN(_d.getTime())) { lastSync = _d.toLocaleDateString(); }
            }
            var groups     = (deviceInfo && deviceInfo.groups) || [];

            if (deviceBannerOs)          deviceBannerOs.textContent = os + (ver ? " · " + ver : "");
            if (deviceBannerCompliance) {
                deviceBannerCompliance.textContent = compliance ? "Compliance: " + compliance : "";
                deviceBannerCompliance.className = "device-banner-compliance compliance-" + (compliance || "unknown").toLowerCase();
            }
            if (deviceBannerUser) deviceBannerUser.textContent = user ? "\uD83D\uDC64 " + user : "";
            if (deviceBannerSync) deviceBannerSync.textContent = lastSync ? "Last sync: " + lastSync : "";
            if (deviceBannerGroups) {
                if (groups.length > 0) {
                    var names = groups.slice(0, 5).map(function (g) { return g.displayName || g.id; }).join(", ");
                    deviceBannerGroups.textContent = "Member of " + groups.length + " group" + (groups.length !== 1 ? "s" : "") + ": " + names + (groups.length > 5 ? " +" + (groups.length - 5) + " more" : "");
                } else {
                    deviceBannerGroups.textContent = "No group memberships found";
                }
            }
            deviceBanner.style.display = "";
        }
    }

    // ── Device mode: select device ──────────────────────────────────────

    async function selectDevice(device) {
        activeGroupId = null;
        nestedData = null;
        orphanedData = null;
        var groupsTab = categoryTabs ? categoryTabs.querySelector('[data-category="groups"]') : null;
        if (groupsTab) groupsTab.style.display = ""; // show Groups tab in device mode
        var membersTabDev = categoryTabs ? categoryTabs.querySelector('[data-category="members"]') : null;
        if (membersTabDev) membersTabDev.style.display = "none";
        ...
        showPanel("loading");
        try {
            assignmentData = await apiFetch("/api/devices/" + device.id + "/assignments");
            // Expose device group memberships as the "groups" category
            assignmentData.groups = (assignmentData._deviceInfo && assignmentData._deviceInfo.groups) || [];
            populateDeviceHeader(device, assignmentData._deviceInfo);
            updateCounts();
            ...
            if (contentErrorMsg) contentErrorMsg.textContent = err.message || "Failed to load device assignments.";
            showPanel("error");
        }
    }
    ...
    function setMode(mode) {
        activeMode = mode;
        var isDevices = (mode === "devices");
        // Set data-mode so CSS rules enforce panel visibility as a safety net
        var sidebarEl = document.getElementById("sidebar");
        if (sidebarEl) sidebarEl.dataset.mode = mode;

        if (btnModeGroups)  btnModeGroups.classList.toggle("active", !isDevices);
        if (btnModeDevices) btnModeDevices.classList.toggle("active",  isDevices);

        // Group-only elements
        if (groupSearchBox)      groupSearchBox.style.display      = isDevices ? "none" : "";
        if (groupHeaderActions)  groupHeaderActions.style.display  = isDevices ? "none" : "";
        var countFilterEl = document.getElementById("countFilter");
        if (countFilterEl) countFilterEl.style.display = isDevices ? "none" : "";
        var stickyList = document.getElementById("groupListSticky");
        if (stickyList) stickyList.style.display = isDevices ? "none" : "";
        var groupListEl = document.getElementById("groupList");
        if (groupListEl) groupListEl.style.display = isDevices ? "none" : "";
        // Also hide the groups-loading and groups-error spinners/banners
        // so they don't bleed through while the Devices panel is active.
        if (sidebarLoading) sidebarLoading.style.display = isDevices ? "none" : (groupListEl && groupListEl.innerHTML === "" ? "flex" : "none");
        if (sidebarError)   sidebarError.style.display   = isDevices ? "none" : "";

        // Device-only elements
        if (deviceSearchPanel) deviceSearchPanel.style.display = isDevices ? "" : "none";
    ...
    if (btnModeGroups)  btnModeGroups.addEventListener("click",  function () { setMode("groups"); });
    if (btnModeDevices) btnModeDevices.addEventListener("click", function () { setMode("devices"); });

    async function loadDevices() {
        // Show a clean loading state with hint text and spinner
        if (deviceSearchHint) {
            deviceSearchHint.textContent = "Loading devices...";
            deviceSearchHint.style.display = "";
        }
        if (deviceSearchLoading) deviceSearchLoading.style.display = "";
        if (deviceList) deviceList.innerHTML = "";
        try {
            var corpParam = deviceOwnershipFilter === "corporate" ? "?corporate=1" : "";
            var devices = await apiFetch("/api/devices" + corpParam);
            if (!Array.isArray(devices)) throw new Error("Unexpected response from server.");
            allDevices    = devices;
            devicesLoaded = true;
            renderDeviceList();
        } catch (err) {
            if (deviceSearchHint) {
                deviceSearchHint.textContent = "Failed to load devices.";
                deviceSearchHint.style.display = "";
            }
            if (deviceList) deviceList.innerHTML = '<li class="group-item group-item-empty">Error: ' + escapeHtml(err.message || "Unknown error") + '</li>';
        } finally {
            if (deviceSearchLoading) deviceSearchLoading.style.display = "none";
        }
    }

    function renderDeviceList() {
        var query    = deviceSearch ? deviceSearch.value.trim().toLowerCase() : "";
        var filtered = allDevices;
        if (query.length >= 1) {
            filtered = allDevices.filter(function (d) {
                return (d.deviceName || "").toLowerCase().indexOf(query) !== -1 ||
                       (d.userPrincipalName || "").toLowerCase().indexOf(query) !== -1;
            });
        }
        if (deviceList) {
            deviceList.innerHTML = "";
            if (filtered.length === 0) {
                deviceList.innerHTML = '<li class="group-item group-item-empty">No devices found</li>';
            } else {
                filtered.forEach(function (device) {
                    var li = document.createElement("li");
                    li.className = "group-item";
                    var osText   = device.operatingSystem || "";
                    var userText = device.userPrincipalName ? " \u2022 " + device.userPrincipalName : "";
                    li.innerHTML = '<span class="group-item-name">' + escapeHtml(device.deviceName || "Unknown") + '</span>' +
                                   '<span class="group-item-meta">' + escapeHtml(osText + userText) + '</span>';
                    li.addEventListener("click", function () {
                        deviceList.querySelectorAll(".group-item").forEach(function (el) { el.classList.remove("active"); });
        ...
        if (deviceSearchHint) {
            deviceSearchHint.textContent = filtered.length + " device" + (filtered.length === 1 ? "" : "s");
            deviceSearchHint.style.display = "";
        }
        // Always hide spinner when the list has been rendered
        if (deviceSearchLoading) deviceSearchLoading.style.display = "none";
    }

    var _deviceSearchTimer = null;
    if (deviceSearch) {
        deviceSearch.addEventListener("input", function () {
            clearTimeout(_deviceSearchTimer);
            _deviceSearchTimer = setTimeout(function () { renderDeviceList(); }, 150);
        });
    }

    // Ownership filter buttons
    ["btnOwnershipCorporate", "btnOwnershipAll"].forEach(function (btnId) {
        var btn = document.getElementById(btnId);
        if (!btn) return;
        btn.addEventListener("click", function () {
            var newFilter = btn.dataset.ownership; // "corporate" or "all"
            if (newFilter === deviceOwnershipFilter) return;
            deviceOwnershipFilter = newFilter;
            // Update active state
            document.querySelectorAll(".device-ownership-btn").forEach(function (b) {
                b.classList.toggle("active", b.dataset.ownership === newFilter);
            });
    ...
    async function selectGroup(group) {
        ...
        // Show Members tab only for real (non-synthetic) groups
        var membersTab = categoryTabs.querySelector('[data-category="members"]');
        if (membersTab) membersTab.style.display = (!group._synthetic) ? "" : "none";
        ...
        try {
            if (group._synthetic && group.id === "__orphaned__") {
                ...
                var backendGroupResults = await Promise.all([
                    ...
                    apiFetch("/api/groups/" + group.id + "/members").catch(function () { return null; })
                ]);
                assignmentData = backendGroupResults[0];
                nestedData = backendGroupResults[1];
                groupMembersData = backendGroupResults[2];
            }
    ...
    var CATEGORIES = [
        ...
        { key: "autopilotProfiles",      countId: "countAutopilotProfiles"      },
        { key: "enrollmentRestrictions", countId: "countEnrollmentRestrictions" },
        { key: "esp",                    countId: "countEsp"                    },
        { key: "featureUpdates",         countId: "countFeatureUpdates"         },
        { key: "qualityUpdates",         countId: "countQualityUpdates"         },
        { key: "driverUpdates",          countId: "countDriverUpdates"          },
        { key: "endpointSecurity",       countId: "countEndpointSecurity"       },
        { key: "diskEncryption",         countId: "countDiskEncryption"         },
        { key: "attackSurfaceReduction", countId: "countAttackSurfaceReduction" },
        { key: "groupPolicyConfigs",     countId: "countGroupPolicyConfigs"     },
        { key: "members",         countId: "countMembers"         },
    ...
    function updateCounts() {
        ...
        CATEGORIES.forEach(function (c) {
            ...
            if (el) {
                if (c.key === "members") {
                    el.textContent = Array.isArray(groupMembersData) ? groupMembersData.length : 0;
                    el.title = "";
                } else if (errors[c.key]) {
                    el.textContent = "!";
    ...
    function getIntuneUrl(category, itemId) {
        ...
        switch (category) {
            ...
            case "autopilotProfiles":
                return INTUNE_BASE + "#view/Microsoft_Intune_Enrollment/AutopilotDevicesMenuBlade/~/deploymentProfiles";
            case "enrollmentRestrictions":
                return INTUNE_BASE + "#view/Microsoft_Intune_Enrollment/EnrollmentRestrictions";
            case "esp":
                return INTUNE_BASE + "#view/Microsoft_Intune_Enrollment/EnrollmentStatusPageBlade";
            case "featureUpdates":
                return INTUNE_BASE + "#view/Microsoft_Intune_Enrollment/WindowsQualityUpdatesMenuBlade/~/featureUpdates";
            case "qualityUpdates":
                return INTUNE_BASE + "#view/Microsoft_Intune_Enrollment/WindowsQualityUpdatesMenuBlade";
            case "driverUpdates":
                return INTUNE_BASE + "#view/Microsoft_Intune_Enrollment/WindowsDriverUpdatesMenuBlade";
            case "endpointSecurity":
                return INTUNE_BASE + "#view/Microsoft_Intune_EndpointSecurity/EndpointSecurityMenu/~/intents";
            case "diskEncryption":
                return INTUNE_BASE + "#view/Microsoft_Intune_EndpointSecurity/EndpointSecurityMenu/~/intents";
            case "attackSurfaceReduction":
                return INTUNE_BASE + "#view/Microsoft_Intune_EndpointSecurity/EndpointSecurityMenu/~/intents";
            case "groupPolicyConfigs":
                return INTUNE_BASE + "#view/Microsoft_Intune_GroupPolicy/GroupPolicyConfigurationsMenuBlade";
            case "groups":
    ...
    function renderCards() {
        ...
        if (activeCategory === "members") {
            cardGrid.innerHTML = "";
            var existingBannerM = document.getElementById("categoryErrorBanner");
            if (existingBannerM) existingBannerM.remove();
            var members = groupMembersData;
            if (!members) {
                cardGrid.style.display = "none";
                categoryEmpty.style.display = "flex";
                return;
            }
            var count = Array.isArray(members) ? members.length : 0;
            if (count === 0) {
                cardGrid.style.display = "none";
                categoryEmpty.style.display = "flex";
                return;
            ...
            var entraBase = "https://entra.microsoft.com/#view/Microsoft_AAD_IAM/";
            members.forEach(function (m) {
                var kind      = (m.kind || "Other").toLowerCase();
                var kindLabel = m.kind || "Other";
                var name      = m.displayName || m.upn || m.id || "Unknown";
                var subtitle  = (kind === "user" && m.upn && m.upn !== name) ? m.upn : "";
                var entraUrl  = kind === "user"
                    ? entraBase + "UserDetailsMenuBlade/~/Overview/userId/" + encodeURIComponent(m.id || "")
                    : kind === "group"
                    ? entraBase + "GroupDetailsMenuBlade/~/Overview/groupId/" + encodeURIComponent(m.id || "")
                    : null;
                var nameHtml = entraUrl
                    ? '<a href="' + escapeHtml(entraUrl) + '" target="_blank" rel="noopener noreferrer" title="Open in Entra">' + escapeHtml(name) + linkIcon + '</a>'
                    : escapeHtml(name);
                var badgeClass = kind === "user" ? "badge-include"
                    : kind === "device" ? "badge-platform"
                    : kind === "group" ? "badge-filter"
                    : "badge-intent";
                ...
                card.innerHTML =
                    ...
                    '<div class="card-meta"><span class="badge ' + badgeClass + '">' + escapeHtml(kindLabel) + '</span></div>';
                var cardActions = card.querySelector(".card-actions");
                (function (n) { cardActions.appendChild(createCopyButton(function () { return n; })); }(name));
        ...
        // ── Device mode: group membership list ─────────────────────────
        if (activeCategory === "groups" && activeMode === "devices") {
            items.forEach(function (group) {
                var card = document.createElement("div");
                card.className = "assignment-card";
                var groupName = group.displayName || group.id || "Unknown Group";
                var entraUrl = "https://entra.microsoft.com/#view/Microsoft_AAD_IAM/GroupDetailsMenuBlade/~/Overview/groupId/" + encodeURIComponent(group.id || "");
                var linkIcon = '<svg class="link-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>';
                var nameHtml = '<a href="' + escapeHtml(entraUrl) + '" target="_blank" rel="noopener noreferrer" title="Open in Entra">' + escapeHtml(groupName) + linkIcon + '</a>';
                card.innerHTML =
                    '<div class="card-header">' +
                        '<div class="card-name">' + nameHtml + '</div>' +
                        '<div class="card-actions"></div>' +
                    '</div>' +
                    '<div class="card-meta"><span class="badge badge-platform">Security Group</span></div>';
                var cardActions = card.querySelector(".card-actions");
                (function (name) {
                    cardActions.appendChild(createCopyButton(function () { return name; }));
                }(groupName));
                cardGrid.appendChild(card);
            });
            return;
        ...
        items.forEach(function (item) {
            var card = document.createElement("div");
            card.className = "assignment-card card--" + activeCategory;
            ...
            if (item.viaGroup) {
                badges.push('<span class="badge badge-via-group" title="Applied via device group membership">via: ' + escapeHtml(item.viaGroup) + '</span>');
            }
            if (item.platform) {
...
var GraphClient = (function () {
    ...
    function detectPlatform(item, categoryKey) {
        if (
            categoryKey === "scripts" ||
            categoryKey === "remediations" ||
            categoryKey === "autopilotProfiles" ||
            categoryKey === "esp" ||
            categoryKey === "featureUpdates" ||
            categoryKey === "qualityUpdates" ||
    ...
    function categoryMatchesItem(item, categoryKey) {
        if (categoryKey !== "enrollmentRestrictions" && categoryKey !== "esp") {
            return true;
        }
        var odataType = ((item && item["@odata.type"]) || "").toLowerCase();
        if (!odataType) return false;
        if (categoryKey === "enrollmentRestrictions") {
            return odataType.indexOf("platformrestrictionsconfiguration") !== -1 ||
                   odataType.indexOf("enrollmentlimitconfiguration") !== -1;
        }
        return odataType.indexOf("enrollmentcompletionpageconfiguration") !== -1;
    }
    ...
    async function getAssignmentsForCategory(url, groupId, categoryKey) {
        ...
        var seenKeys = {};
        items.forEach(function (item) {
            if (!categoryMatchesItem(item, categoryKey)) return;
            ...
            assignments.forEach(function (a) {
                ...
                if (info.match) {
                    var tgt = (a.target) || {};
                    var dedupKey = item.id + "|" + (tgt["@odata.type"] || "") + "|" + (tgt.groupId || "");
    ...
    async function getAssignmentsByTargetType(targetOdataType, label) {
        var endpoints = {
            ...
            autopilotProfiles: GRAPH_BASE + "/beta/deviceManagement/windowsAutopilotDeploymentProfiles?$expand=assignments&$select=id,displayName,description,assignments",
            enrollmentRestrictions: GRAPH_BASE + "/beta/deviceManagement/deviceEnrollmentConfigurations?$expand=assignments&$select=id,displayName,description,assignments",
            esp: GRAPH_BASE + "/beta/deviceManagement/deviceEnrollmentConfigurations?$expand=assignments&$select=id,displayName,description,assignments",
            featureUpdates: GRAPH_BASE + "/beta/deviceManagement/windowsFeatureUpdateProfiles?$expand=assignments&$select=id,displayName,description,assignments",
            qualityUpdates: GRAPH_BASE + "/beta/deviceManagement/windowsQualityUpdateProfiles?$expand=assignments&$select=id,displayName,description,assignments",
            driverUpdates: GRAPH_BASE + "/beta/deviceManagement/windowsDriverUpdateProfiles?$expand=assignments&$select=id,displayName,description,assignments"
        };
        ...
        var promises = keys.map(function (key) {
            return graphFetchAll(endpoints[key]).then(function (items) {
                ...
                var seenKeys = {};
                items.forEach(function (item) {
                    if (!categoryMatchesItem(item, key)) return;
                    var platform = detectPlatform(item, key);
                    (item.assignments || []).forEach(function (a) {
                        var tgt = a.target || {};
                        var t = tgt["@odata.type"] || "";
                        if (t === targetOdataType) {
                            var dedupKey = item.id + "|" + t + "|" + (tgt.groupId || "");
                            if (seenKeys[dedupKey]) return;
                            seenKeys[dedupKey] = true;
    ...
    async function getNestedAssignments(groupId) {
        ...
        if (!parents || parents.length === 0) {
            return {
                ...
                scripts: [], remediations: [], autopilotProfiles: [],
                enrollmentRestrictions: [], esp: [], featureUpdates: [],
                qualityUpdates: [], driverUpdates: [], _errors: {}
            };
        }
                ...
                items.forEach(function (item) {
                    ...
                    (item.assignments || []).forEach(function (a) {
                        ...
                        if (tGroupId && parentLookup[tGroupId]) {
                            ...
                            var dedupKey = item.id + "|" + odataType + "|" + tGroupId;
                            if (seenKeys[dedupKey]) return;
                            seenKeys[dedupKey] = true;
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            if ($path -eq "/api/status" -and ($req.HttpMethod -eq "GET" -or $req.HttpMethod -eq "HEAD")) {
                ...
                $statusAccount  = if ($mgCtx -and $mgCtx.Account)  { "$($mgCtx.Account)"  } else { "" }
                $statusTenantId = if ($mgCtx -and $mgCtx.TenantId) { "$($mgCtx.TenantId)" } else { "" }
                $statusObj  = @{ mode = "backend"; account = $statusAccount; tenantId = $statusTenantId }
                $statusBody = ConvertTo-Json -InputObject $statusObj -Compress
            ...
            # -- API: group members ---------------------------------
            elseif ($path -match "^/api/groups/([^/]+)/members$" -and $req.HttpMethod -eq "GET") {
                $groupId    = $Matches[1]
                $parsedGuid = [System.Guid]::Empty
                if (-not [System.Guid]::TryParse($groupId, [ref]$parsedGuid)) {
                    $resp.StatusCode = 400
                ...
                try {
                    ...
                    $json    = ConvertTo-SafeJson -InputObject $members -AsArray
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($json)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    $resp.StatusCode      = 200
                    ...
                    Write-Warning "Group members fetch failed for $groupId : $($_.Exception.Message)"
                    $errBody = ConvertTo-Json -InputObject @{ error = "Failed to fetch group members." } -Compress
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($errBody)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    $resp.StatusCode      = 502
            ...
            # -- API: all devices list ------------------------------
            elseif ($path -eq "/api/devices" -and $req.HttpMethod -eq "GET") {
                try {
                    $corporateOnly = ($req.QueryString["corporate"] -eq "1")
                    $devices = @(Get-AllManagedDevices -CorporateOnly:$corporateOnly)
                    $json    = ConvertTo-SafeJson -InputObject $devices -AsArray
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($json)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    $resp.StatusCode      = 200
                    ...
                    Write-Warning "Failed to fetch devices: $($_.Exception.Message)"
                    $errBody = ConvertTo-Json -InputObject @{ error = "Failed to fetch devices: $($_.Exception.Message)" } -Compress
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($errBody)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    $resp.StatusCode      = 502
            ...
            # -- API: device search ----------------------------------
            elseif ($path -eq "/api/devices/search" -and $req.HttpMethod -eq "GET") {
                $query = $req.QueryString["q"]
                if (-not $query -or $query.Trim().Length -lt 2) {
                    $resp.StatusCode = 400
                ...
                try {
                    $corporateOnly = ($req.QueryString["corporate"] -eq "1")
                    $devices = @(Get-ManagedDevicesByName -SearchName $query.Trim() -CorporateOnly:$corporateOnly)
                    $json    = ConvertTo-SafeJson -InputObject $devices -AsArray
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($json)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    ...
                    Write-Warning "Device search failed: $($_.Exception.Message)"
                    $errBody = ConvertTo-Json -InputObject @{ error = "Device search failed: $($_.Exception.Message)" } -Compress
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($errBody)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    $resp.StatusCode      = 502
            ...
            elseif ($path -match "^/api/devices/([^/]+)/assignments$" -and $req.HttpMethod -eq "GET") {
                $deviceId   = $Matches[1]
                $parsedGuid = [System.Guid]::Empty
                if (-not [System.Guid]::TryParse($deviceId, [ref]$parsedGuid)) {
                    $resp.StatusCode = 400
                    $body   = '{"error":"Invalid device ID format. Expected a valid GUID."}'
                    $buffer = [System.Text.Encoding]::UTF8.GetBytes($body)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    $resp.OutputStream.Write($buffer, 0, $buffer.Length)
                ...
                $deviceId = $parsedGuid.ToString()
                try {
                    $assignmentsResult = Get-AssignmentsForDevice -ManagedDeviceId $deviceId
                    $json   = ConvertTo-SafeJson -InputObject $assignmentsResult
                    $buffer = [System.Text.Encoding]::UTF8.GetBytes($json)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    ...
                    Write-Warning "Device assignment fetch failed for $deviceId : $($_.Exception.Message)"
                    $errBody = ConvertTo-Json -InputObject @{ error = "Failed to fetch device assignments: $($_.Exception.Message)" } -Compress
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($errBody)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
                    $resp.StatusCode      = 502
