...
function Get-ManagedDevicesByName {
    ...
    $uri = "/beta/deviceManagement/managedDevices?`$filter=startsWith(deviceName,'$encoded')&`$select=id,deviceName,azureADDeviceId,operatingSystem,osVersion,userPrincipalName,lastSyncDateTime,complianceState&`$top=50"
...
function Get-AllManagedDevices {
    ...
    #>
    $uri = "/beta/deviceManagement/managedDevices?`$select=id,deviceName,operatingSystem,osVersion,userPrincipalName,lastSyncDateTime,complianceState&`$top=100"
...
(function () {
    ...
    async function loadDevices() {
        ...
        try {
            var devices = await apiFetch("/api/devices");
    ...
    while ($listener.IsListening) {
        ...
        try {
            ...
            elseif ($path -eq "/api/devices/search" -and $req.HttpMethod -eq "GET") {
                ...
                try {
                    $devices = @(Get-ManagedDevicesByName -SearchName $query.Trim())
                    $json    = ConvertTo-SafeJson -InputObject $devices -AsArray
                    $buffer  = [System.Text.Encoding]::UTF8.GetBytes($json)
                    $resp.ContentType     = "application/json; charset=utf-8"
                    $resp.ContentLength64 = $buffer.Length
