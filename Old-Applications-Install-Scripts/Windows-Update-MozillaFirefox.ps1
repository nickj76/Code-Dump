<# 
.SYNOPSIS
   Check the version of Mozilla Firefox and Determine the architecture of a machine then compare to latest version avaliable from the Mozilla website

.DESCRIPTION
   After performing checks to determine the installed version of Firefox, the machines architecture and the language it will then download and perform an upgrade.
   If Firefox is not installed the script will exit.  

.EXAMPLE
   PS C:\> .\Windows-Update-MozillaFirefox.ps1
...
If ($null -ne (Check-InstalledSoftware "*Firefox*")) {
...
If ($null -ne $registry_paths_selection) {
    ...
    ForEach ($item in $registry_paths_selection) {
       ...
       # $product_version_enum = ((Get-ItemProperty -Path "C:\Program Files (x86)\Mozilla Firefox\Firefox.exe" -ErrorAction SilentlyContinue -Name VersionInfo).VersionInfo).ProductVersion
        ...
        If (($null -ne $product_version_enum) -and ($test_stability -eq $true)) { $product_version_enum -match "(?<C1>\d+)\.(?<C2>\d+)\.(?<C3>\d+)" | Out-Null } Else { $continue = $true }
        If (($null -ne $product_version_enum) -and ($test_stability -eq $false) -and ($test_major -eq $true)) { $product_version_enum -match "(?<C1>\d+)\.(?<C2>\d+)" | Out-Null } Else { $continue = $true }
                            ...
                            $firefox_enumeration += $obj_firefox = New-Object -TypeName PSCustomObject -Property @{
                                ...
                                'Type'                          = $type
                                'Locale'                        = $locale
                                'Major Version'                 = If ($null -ne $Matches.C1) { $Matches.C1 } Else { $continue = $true }
                                'Minor Version'                 = If ($null -ne $Matches.C2) { $Matches.C2 } Else { $continue = $true }
                                'Build Number'                  = If ($null -ne $Matches.C3) { $Matches.C3 } Else { "-" }
                                'Computer'                      = $computer
        ...
        If ($null -ne $firefox_enumeration) {
...
If ((($PSVersionTable.PSVersion).Major -lt 3) -or ($null -eq ($PSVersionTable.PSVersion).Major)) {
    ...
    $powershell_v2_or_earlier = $true
            ...
            If ($null -eq ($PSVersionTable.PSVersion).Major) {
                $powershell_v1 = $true
    ...
    # Source: https://msdn.microsoft.com/en-us/powershell/reference/5.1/microsoft.powershell.utility/convertfrom-stringdata
    # Source: https://technet.microsoft.com/en-us/library/ee692803.aspx
    If ($null -eq $latest_release_date) {
...
If (($multiple_firefoxes -ne $true) -and ($admin_corner -ne $true)) {
            ...
            If ($null -ne ($firefox_enumeration | Select-Object -ExpandProperty Locale)) {
                $lang = [string]'&lang=' + ($firefox_enumeration | Select-Object -ExpandProperty Locale)
            } ElseIf ($null -eq ($firefox_enumeration | Select-Object -ExpandProperty Locale)) {
...
If ($null -ne $registry_paths_after_update) {
    ...
    ForEach ($new_firefox in $registry_paths_after_update) {
        ...
        If (($null -ne $product_version_new) -and ($regex_stability -eq $true)) { $product_version_new -match "(?<B1>\d+)\.(?<B2>\d+)\.(?<B3>\d+)" } Else { $continue = $true }
        If (($null -ne $product_version_new) -and ($regex_stability -eq $false) -and ($regex_major -eq $true))  { $product_version_new -match "(?<B1>\d+)\.(?<B2>\d+)" } Else { $continue = $true }
                            ...
                            $after_update_firefoxes += $obj_updated_firefox = New-Object -TypeName PSCustomObject -Property @{
                                ...
                                'Type'                          = $type_new
                                'Locale'                        = $locale_new
                                'Major Version'                 = If ($null -ne $Matches.B1) { $Matches.B1 } Else { $continue = $true }
                                'Minor Version'                 = If ($null -ne $Matches.B2) { $Matches.B2 } Else { $continue = $true }
                                'Build Number'                  = If ($null -ne $Matches.B3) { $Matches.B3 } Else { "-" }
                                'Computer'                      = $computer
        ...
        # Display the Firefox Version Enumeration in console
        If ($null -ne $after_update_firefoxes) {
...
If (($firefox_is_installed -eq $true) -and ($downloading_firefox_is_required -eq $true) -and ($null -ne $after_update_firefoxes) -and ($multiple_firefoxes_after_update -eq $false)) {

    If ($null -eq $most_recent_firefox_after_update) {
        $success = $false
        $empty_line | Out-String
...
If ($null -ne $obj_success_firefox) {
