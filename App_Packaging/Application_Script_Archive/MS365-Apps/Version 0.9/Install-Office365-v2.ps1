<# 
.SYNOPSIS
   Install Script for Office365

.DESCRIPTION
   Script that installs Office 365 using parameters that you specify to install to your specific needs.
...
$ConfigurationXMLFile = "C:\Temp\365apps-with-Visio-user-based.xml"
...
try {
  ...
  Start-Process "$OfficeInstallDownloadPath\Setup.exe" -ArgumentList "/configure $ConfigurationXMLFile" -Wait -PassThru --Changed 29/11/21
  #Start-Process "$OfficeInstallDownloadPath\Setup.exe" -ArgumentList "/setup $ConfigurationXMLFile" -Wait -PassThru
}
catch {
