...
Function updateWindows {
	...
	Try{
		...
		if ($null -ne $updates){			
			...
			$script = Import-Module PSWindowsUpdate; Install-WindowsUpdate -AcceptAll -IgnoreReboot | Out-File $logFile
			...
			do {$updatestatus = Get-Content $DownloadDir\PSWindowsUpdate.log
				...
				$updatetimeout++
				Write-Output $installednumber
				Write-Output $Failednumber
