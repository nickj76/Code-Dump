...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		# Check if already installed and update if needed (before running driver updates)
		If (Get-InstalledApplication -Name "$appRegName"){
			[string]$appDisplayVersion = (Get-InstalledApplication -Name "$appRegName") | Select-Object DisplayVersion -expand DisplayVersion
			
			#Verify if installed version is the latest
			If ($appDisplayVersion -lt $appVersion) {
			
				#Stop the Dell Update Service before running install 
				Stop-Service -Name "DellClientManagementService" -Force

				#Install the Dell Command | Update Utility 
				Show-InstallationProgress "Updating $appRegName... Please Wait..." 
				$ReturnCode = Execute-Process -Path "$dirFiles\$appFile" -Parameters "/s" -WindowStyle Hidden -PassThru -ContinueOnError $true -IgnoreExitCodes '*'
		
				# Error you sometimes get when app wants a system restart before running installer
				If ($ReturnCode -like "*ExitCode=2*"){
					Show-InstallationPrompt -Message "ERROR: $appRegName Requires a System Restart... Please Restart Computer and Relaunch..." -ButtonRightText 'EXIT' -Icon Information -NoWait
					Exit $mainExitCode
				}
				
				Show-InstallationProgress "Waiting for $appRegName Install... Please Wait..." 

				#Wait for Actual DCU Installer to finish 
				Wait-Process -Name "DCU_Setup_*" -Timeout 200

				#Start the Dell Update Service 
				Start-Service -Name "DellClientManagementService"

				# Waiting for DellClientManagementService service (waiting incase something hasn't finished before running scan)
				Start-sleep -s 10
			}
			Else{
				Show-InstallationProgress "$appRegName Already Installed, Moving to Driver Update Phase...  Please Wait..."
		...
		Else{
			#Install the Dell Command | Update Utility 
			Show-InstallationProgress "Installing $appRegName... Please Wait..." 
			$ReturnCode = Execute-Process -Path "$dirFiles\$appFile" -Parameters "/s" -WindowStyle Hidden -PassThru -ContinueOnError $true -IgnoreExitCodes '*'
