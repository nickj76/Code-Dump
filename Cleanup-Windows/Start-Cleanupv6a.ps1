...
param(
    ...
    ## Removes c:\users\*\Application Data\SPSSInc\Graphboard
    if (test-path c:\users\*\Application Data\SPSSInc\Graphboard){
        remove-item -Path c:\users\*\Application Data\SPSSInc\Graphboard -force -recurse -Verbose -ErrorAction SilentlyContinue
    } else {
        Write-Host "c:\users\*\Application Data\SPSSInc\Graphboard does not exist, there is nothing to cleanup.                                        " -NoNewline -ForegroundColor DarkGray
        Write-Host "[WARNING]" -ForegroundColor DarkYellow -BackgroundColor Black
    }
