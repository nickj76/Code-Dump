...
param(
    ...
    ## Removes C:\users\*\AppData\Roaming\SPSSInc\Graphboard
    if (test-path C:\users\*\AppData\Roaming\SPSSInc\Graphboard){
        remove-item -Path C:\users\*\AppData\Roaming\SPSSInc\Graphboard -force -recurse -Verbose -ErrorAction SilentlyContinue
    } else {
        Write-Host "C:\users\*\AppData\Roaming\SPSSInc\Graphboard does not exist, there is nothing to cleanup.                                          " -NoNewline -ForegroundColor DarkGray
        Write-Host "[WARNING]" -ForegroundColor DarkYellow -BackgroundColor Black
    }

    ## Removes C:\Users\*\Application Data\Microsoft\Forms
    if (test-path "C:\Users\*\Application Data\Microsoft\Forms"){
        remove-item -Path "C:\Users\*\Application Data\Microsoft\Forms" -force -recurse -Verbose -ErrorAction SilentlyContinue
    } else {
        Write-Host "C:\Users\*\Application Data\Microsoft\Forms does not exist, there is nothing to cleanup.                                          " -NoNewline -ForegroundColor DarkGray
        Write-Host "[WARNING]" -ForegroundColor DarkYellow -BackgroundColor Black
    }
    ...
    if (Test-path 'C:\$Windows.BT'){
        ...
        ## Removes the C:\$windows.WS.
    if (Test-path 'C:\$Windows.WS'){
        Remove-Item 'C:\$Windows.WS' -Recurse -Force -Verbose -ErrorAction SilentlyContinue
    } else {
