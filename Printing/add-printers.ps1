...
$printers = @()
$printers += [pscustomobject]@{PrinterName="SurreyPrint";PrintServer="\\printservice.surrey.ac.uk\surreyprint";ADGroup="";Default="1"}
...
if (-not (Test-RunningAsSystem)) {
    ...
    foreach ($printer in $Printers) { 
        if($null -ne $printer.ADGroup -and $printer.ADGroup.Contains(",")) { 
