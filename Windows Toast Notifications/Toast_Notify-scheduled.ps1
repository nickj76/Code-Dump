...
If ($XMLValid -eq $True) {
    ...
    If (([System.Security.Principal.WindowsIdentity]::GetCurrent()).Name -eq "NT AUTHORITY\SYSTEM") {
        ...
        $Task_Trigger = New-ScheduledTaskTrigger -Weekly -DaysOfWeek Friday -At 3pm $Task_TimeToRun
        $Task_Trigger.EndBoundary = $Task_Expiry
        $Task_Principal = New-ScheduledTaskPrincipal -GroupId "S-1-5-32-545" -RunLevel Limited
        $Task_Settings = New-ScheduledTaskSettingsSet -Compatibility V1 -DeleteExpiredTaskAfter (New-TimeSpan -Seconds 2,419,200) -AllowStartIfOnBatteries
