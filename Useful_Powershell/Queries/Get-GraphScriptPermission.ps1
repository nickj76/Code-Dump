...
.DESCRIPTION
The Get-GraphScriptPermission function takes a scriptblock as input and parses it to extract a list of
command elements and their associated parameters. It then filters the list to include only the commands
sourced from Microsoft.Graph. For each Microsoft.Graph command, it calls the Find-MgGraphCommand function
to retrieve the permissions associated with that command. The function returns an object containing the
command, its source, verb, noun, and the list of permissions with their names and whether they require
admin privileges.
...
function Get-GraphScriptPermission {
    ...
    process {
        foreach($GraphCommand in ($commandElementList | Where-Object Source -like 'Microsoft.Graph*')){            
            $GraphCommand.AllPriviledges = (Find-MgGraphCommand -Command $GraphCommand.Cmdlet).Permissions.ForEach({
                [PSCustomObject]@{
                    Name = $_.Name
                    IsAdmin = $_.IsAdmin
                }
