...
#  [array]$getalldevices = Get-IntuneManagedDevice -Filter "deviceName eq 'APL415'" | Get-MSGraphAllPages 
  $getalldevices = Get-IntuneManagedDevice | Get-MSGraphAllPages 
#  $getalldevices = Get-IntuneManagedDevice -Filter "contains(deviceName, 'APL4')" | Get-MSGraphAllPages 
...
ForEach-Object{
  ...
  #$foundApp = $app.detectedApps | Where-Object{$_.displayname -like $Application}

  [array]$foundApps = $app.detectedApps | Where-Object{$_.displayname -match ".*$Application.*"}
  ...
  #Write-Host $app.detectedApps
  #  $foundApp = $app.detectedApps | Where-Object{$_.displayname -like $Application}
  #$foundApp = $app.detectedApps | Where-Object{$_.displayname -match ".*$Application.*"}
  #$app.detectedApps | Write-Host $_.displayName
...
$collection | where{$_.appname -match ".*$Application.*"} | Export-Csv $Report -NoTypeInformation
