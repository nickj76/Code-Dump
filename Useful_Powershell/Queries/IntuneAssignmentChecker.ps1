...
$appid = 'xxxxx-xxxxxxx-xxxxxx-xxxxx' # App ID of the App Registration
$tenantid = 'xxxxx-xxxxxxx-xxxxxx-xxxxx' # Tenant ID of your EntraID
$secret = 'xxxxx-xxxxxxx-xxxxxx-xxxxx' # Secret of the App Registration
    ...
    switch ($selection) {
        ...
        '2' {
            ...
            foreach ($GroupName in $GroupNames) {
                ...
                $GroupRelevantScripts = @()
                $GroupRelevantRemediations = @()
                ...
                # Get Intune Scripts
                $deviceScripts = Get-MgBetaDeviceManagementScript

                # Check each device script for assignments that match devices groups
                foreach ($script in $deviceScripts) {
                    $scriptName = $script.displayName
                    $scriptId = $script.id

                    $assignments = Get-MgBetaDeviceManagementScriptGroupAssignment -devicemanagementscriptid $scriptId

                    foreach ($assignment in $assignments) {
                        if ($entragroupId -contains $assignment.targetGroupId) {
                            $GroupRelevantScripts += $script
                            break
                        }
                    }
                }

                # Get Intune Remediatons
                $deviceRemediations = Get-MgBetaDeviceManagementDeviceHealthScript

                # Check each device remediation for assignments that match devices groups
                foreach ($remediation in $deviceRemediations) {
                    $remediationName = $remediation.displayName
                    $remediationId = $remediation.id

                    $assignments = Get-MgBetaDeviceManagementDeviceHealthScriptAssignment -devicehealthscriptid $remediationId

                    foreach ($assignment in $assignments) {
                        $assignTarget = $assignment.target
                        $targetGroupId =  $assignTarget.additionalProperties["groupId"]

                        if ($entragroupId -contains $targetGroupId) {
                            #write-output "Script $scriptName is assigned"
                            $GroupRelevantRemediations += $remediation
                            break
                        }
                    }
                }
                ...
                # Separator and heading for Assigned Scripts
                Write-Host "------- Assigned Scripts Profiles -------" -ForegroundColor Cyan

                foreach ($script in $GroupRelevantScripts) {
                    $scriptName = $script.DisplayName
                    $scriptId = $script.id
                    Write-Host "Script Name: $scriptName, Script ID: $scriptId" -ForegroundColor White
                }


                # Separator and heading for Assigned Remediations
                Write-Host "------- Assigned Remediations -------" -ForegroundColor Cyan

                foreach ($remediation in $GroupRelevantRemediations) {
                    $remediationName = $remediation.DisplayName
                    $remediationId = $remediation.id
                    Write-Host "Remediation Name: $remediationName, Remediation ID: $scriptId" -ForegroundColor White
                }
